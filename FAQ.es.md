...
Porque 1080p60 es el estándar de calidad actual para la mayoría de las consolas y dispositivos de captura modernos. Apuntar a 1080p60 garantiza una buena calidad de imagen y una experiencia de juego fluida.
...
En teoría, sí. Sin embargo, para este proyecto se recomienda una tarjeta de captura que admita 1080p60 y cuente con salida en bucle (entrada HDMI + salida HDMI). Hay opciones relativamente económicas y fáciles de encontrar. Un ejemplo que puedes buscar es: «Captura de vídeo HD 4K Ultra HD USB 3.0 (MS 2131)». Estos dispositivos admiten entradas de hasta 4K60 y emiten a 1080p60. Por ejemplo, si conectas una consola que admite 4K a 60 fps, la tarjeta de captura lo aceptará, pero la imagen final utilizada por MPV-SW-Capture será de 1080p a 60 fps.
...
Sí, funcionará perfectamente. Por ejemplo, se probó con una salida de solo 720p y el resultado fue una mejora automática a 1080p. Con esto, todo funcionó correctamente, incluso los marcos, los recortes y los shaders.
...
### 2. He seguido las instrucciones y he configurado correctamente mi `MPV-SW-Capture`. Sin embargo, me he dado cuenta de que los tres programas tienen ahora una versión más reciente que la que utilizaba antes. ¿Puedo actualizarlos sin que deje de funcionar el MPV-SW-Capture?
...
Cuando ves una actualización, aparecen tres dígitos separados por puntos. Se representan como `X`, `Y`, `Z`:
...
Esto suele deberse a la barra negra que muestra NSO en la parte inferior con los controles y el texto de ayuda. Tienes que desactivar esa superposición. Abre cualquier aplicación de NSO y, antes de seleccionar un juego, ve al «menú lateral derecho» → «Ajustes» → «Control de pantalla» → desactiva «Mostrar controles en el juego». Una vez que desaparezca esa barra, el área de la imagen quedará más limpia (y, en algunos casos, más grande) y los recortes y marcos funcionarán correctamente. Hay que hacerlo en cada aplicación de NSO.

### 4. ¿Cómo desactivo un shader, un marco o un recorte?

- **Shader**: Utiliza la opción «Borrar shader» o, simplemente, selecciona un shader diferente (sustituye al anterior; no se acumulan).
...
- **Forma**: pulsa la misma opción de forma que seleccionaste para desactivarla, o utiliza la opción «Borrar forma».
...
### 3. Cuando grabo un vídeo, ¿se incluyen los marcos, recortes o shaders?
...
Solo tienes que hacer clic con el botón derecho del ratón en la ventana y se abrirá el menú. También puedes pulsar la tecla `ESC` del teclado, pero asegúrate de que el cursor del ratón esté sobre la ventana de MPV‑SW‑Capture y de que el programa tenga el foco.
...
- **Manual** (no recomendado): En Windows, haz clic en el icono de sonido de la bandeja del sistema, abre el panel de salida/mezclador, busca la entrada «ffplay» y ajusta su volumen al nivel que desees. Esto modificará específicamente el volumen de MPV‑SW‑Capture. Utiliza este método solo si el método oficial falla.
...
Cualquier opción, sea cual sea la que elijas, solo se mantendrá hasta que cierres MPV-SW-Capture.

Por lo tanto, si lo vuelves a abrir, todo volverá a las opciones predeterminadas.

Las únicas opciones que puedes guardar para que se inicien automáticamente son algunas de las que puedes seleccionar en **Configuración**, **Instalación** y **Herramientas**.
...
- **_Navegación por el MENÚ_:** También puedes utilizar las teclas `ARRIBA`, `ABAJO`, `IZQUIERDA` y `DERECHA` del teclado para navegar por el menú si lo deseas.
- **_Acceso rápido_:** Tecla `CTRL` + una tecla del `1` al `4`, para cambiar rápidamente a una selección de shaders.
  - `CTRL`+`5` sirve para «Shaders limpios».
  - `CTRL`+`0` sirve para «Clean ALL».
...
La opción **«Ocultar mensajes OSD»** se encuentra en el menú, en **OTROS → Ocultar mensajes OSD**. Cuando está activada, oculta todos los mensajes en pantalla que aparecen al seleccionar cualquier opción (por ejemplo, cambios de shaders, ajustes de recorte, etc.). Esto es ideal para streamers que no quieren que estas notificaciones aparezcan en su emisión, o para usuarios que simplemente prefieren una interfaz más limpia.
...
### 1. Tengo algunos comentarios, incidencias o peticiones sobre este proyecto. ¿Dónde puedo compartir mi opinión en GitHub?
...
Se trata de una limitación que tienen las tarjetas de captura por defecto.
...
En el programa de configuración, pulsa **«Escanear dispositivo»** y elige la otra tarjeta de captura (vídeo y audio) que desees utilizar.
