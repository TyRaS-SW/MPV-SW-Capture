...
# ============================================================
#  FORM ICON (prioritizes data\setupmsc.ico)
# ============================================================
function Get-AppIcon {
    # 1. First, try to load the icon from data\setupmsc.ico
    try {
        $iconPath = Join-Path $script:RootDir 'data\setupmsc.ico'
        if (Test-Path -LiteralPath $iconPath) {
            return New-Object System.Drawing.Icon($iconPath)
        }
    } catch {}

    # 2. If not found, try to extract from the script itself (if it's an .exe)
    try {
    ...
    # 3. Fallback: extract from the PowerShell executable
    try {
...
$script:Lang["EN"] = @{
    ...
    S5Shorcut     = "CREATE DESKTOP AND LOCAL SHORTCUTS FOR MPV-SW-Capture"
    ...
    S5No        = "No  - Do not create shortcuts."
    S5Yes       = "Yes - Create shortcuts."
...
$script:Lang["ES"] = @{
    ...
    S5Shorcut     = "CREAR ACCESOS DIRECTOS EN EL ESCRITORIO Y LOCAL PARA MPV-SW-Capture"
    ...
    S5No        = "No  - No crear accesos directos."
    S5Yes       = "Si  - Crear accesos directos."
...
function Get-AppDir {
    # This is replaced by Get-RootDir, but kept for compatibility
...
function Apply-Lang([string]$lang) {
    ...
    foreach ($card in @($card1,$card2,$card3,$card4,$card5,$card6)) {
        ...
        if ($tL) {
            if ($card.Tag -eq "ACT") { $tL.Text = T "ActionsTitle" }
            else { $tL.Text = T ($card.Tag + "Title") }
        }
    }
...
$btnScan.Add_Click({
    ...
    try {
        ...
        $ffplayCmd   = if (Test-Path $ffplayLocal) { $ffplayLocal } else { "ffplay" }
...
$btnMenuToggle.Add_Click({
    ...
    $mapEN = @{
        'CERRAR MPV-SW-Capture'='CLOSE MPV-SW-Capture'; 'Videojuegos'='VideoGame'; 'Rellenar Ventana'='Fill Screen'; 'Solo Nitidez'='Only Sharpen'; 'Limpiar Shaders'='Clean Shaders'; 'RECORTES'='CROPS'; 'Borrar Recorte'='Clear Crop'; 'Recorte Eliminado'='Crop Cleared'
...
'ROTACION'='ROTATION'; 'Girar a la Derecha'='Rotate Clockwise'; 'Girar a la Izquierda'='Rotate Counter-Clockwise'; 'Restablecer Rotacion'='Reset Rotation'; 'OTRAS OPCIONES'='OTHER OPTIONS'; 'Borde de Ventana'='Window Border'; 'Barra de Titulo'='Title Bar'; 'Quitar TODO'='Remove ALL'; 'Girado +90°'='Rotated +90°'; 'Girado -90°'='Rotated -90°'; 'Rotacion Reiniciada'='Rotation Reset'; 'Borde: ${border}'='Border: ${border}'; 'Barra de Titulo: ${border}'='Title Bar: ${border}'; 'Ambos Quitados'='Removed Both'
    ...
    $mapES = @{
        'CLOSE MPV-SW-Capture'='CERRAR MPV-SW-Capture'; 'VideoGame'='Videojuegos'; 'Fill Screen'='Rellenar Ventana'; 'Only Sharpen'='Solo Nitidez'; 'Clean Shaders'='Limpiar Shaders'; 'CROPS'='RECORTES'; 'Clear Crop'='Borrar Recorte'; 'Crop Cleared'='Recorte Eliminado'
...
$btnApply.Add_Click({
    ...
    try {
        ...
        if ($rbScYes.Checked) {
            ...
            $icoPath   = Join-Path $root "data\msc-shortcut.ico"
...
IconIndex=0
IconFile=$($root)\data\msc-shortcut.ico
"@
