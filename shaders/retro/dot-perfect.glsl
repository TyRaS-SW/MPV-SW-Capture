...
void vertex_main() {
    ...
    c11 = vTexCoord;
    // REVISION DE INTENSIDAD: El escalado define cuan grandes son los "puntos" en pantalla. 
    // Usando 1.25 logramos el "sweet spot" donde los bordes se notan sin comerse la imagen.
    pixel_no = vTexCoord * (global.OutputSize.xy / 1.25);
}
...
void fragment_main() {
    ...
    color += fragment_lookup(pixel_no, 0., 1., texture(Source, vTexCoord).rgb);
    color += fragment_lookup(pixel_no, 1., 1., texture(Source, vTexCoord).rgb);
    // Puedes hacer el efecto mas pronunciado ajustando este "0.65" hacia abajo (ej. 0.45) 
    // o el multiplicador "1.2" de iluminacion.
