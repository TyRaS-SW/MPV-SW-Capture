...
void fragment_main() {
    ...
    // -- FIXED SCALE MATH --
    float scale_raw = params.OutputSize.y * params.SourceSize.w;
    float scale = max(scale_raw, 1.0); // Evitar division por cero (pantalla negra)
    float offset = 0.5 / scale;
    if (mod(floor(scale_raw + 0.5), 2.0) >= 1.0) offset = 0.0;
    ...
    // -- FIXED SCANLINE MATH --
    // Original: fract(co.y * params.SourceSize.y) -> Dependia de la resolucion interna
    // Nuevo: Forzamos a que el ciclo dependa SIEMPRE de la resolucion de tu monitor/ventana
    // '3.0' son los pixeles de grosor que tendra la linea. 
    float grosor_lineas = 6.0; 
    float scan_x = fract((vTexCoord.y * params.OutputSize.y) / grosor_lineas);
    col *= fragment_get_scanline_weight(scan_x, col_soft);
